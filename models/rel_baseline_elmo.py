...
import models.utils
from models.base_models import ElmoClsModel
...
class Baseline:
    ...
    def __init__(self, device, training_mode, **kwargs):
        ...
        self.model = ElmoClsModel(model_name=kwargs.get('model'),
                                         n_classes=1,
        ...
        for epoch in range(n_epochs):
            ...
            for text, label, candidates in dataloader:
                ...
                input_dict = self.model.encode_text(
                    list((replicated_text, replicated_relations)))
                output = self.model(input_dict)
                targets = torch.tensor(
                    ranking_label).float().unsqueeze(1).to(self.device)
        ...
        if self.training_mode == 'sequential':
            for cluster_idx, train_dataset in enumerate(train_datasets):
                ...
                self.train(dataloader=train_dataloader,
                           n_epochs=n_epochs, log_freq=log_freq)
